...
import sys
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QLabel, QTextEdit, QMessageBox
...
backspace_count = 0
SESSION_DURATION = 60
last_key_time = None  # Track the last keypress
...
def on_press(key):
    global start_time, text_buffer, current_keys, backspace_count, last_key_time
    now = time.time()
    last_key_time = now  # update last key press time
...
def monitor_typing():
    global start_time, last_key_time
    while True:
        time.sleep(0.5)
        if start_time and last_key_time:
            if time.time() - last_key_time > IDLE_THRESHOLD:
                analyze_typing()
                reset_state()

def reset_state():
    global keystroke_data, current_keys, text_buffer, start_time, backspace_count, last_key_time
...
def analyze_typing():
    ...
    if superhuman_speed or uniform_typing or low_entropy:
        result.append("⚠️ Suspicious typing detected :")
    ...
    if ui_window:
        ...
        if suspicious:
            QMessageBox.warning(ui_window, "Bot Detected", "\n".join(result))
...
class TypingMonitorUI(QWidget):
    def __init__(self):
        ...
        layout = QVBoxLayout()
        self.label = QLabel("Monitoring typing...")
